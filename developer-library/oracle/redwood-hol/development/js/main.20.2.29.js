...
Version: 20.2.29
Feature added: DBDOC-2455 - Support for content selectable via tabs
*/
"use strict";
...
let main = function() {
    ...
    let loadTutorial = function(articleElement, selectedTutorial, manifestFileContent, callbackFunc=null) {
        $.get(selectedTutorial.filename, function(markdownContent) { //reading MD file in the manifest and storing content in markdownContent variable
            ...
            addNeedHelpLink(manifestFileContent.support); // adding link to the Neep Help URL in the footer if the manifest file contains it (DBDOC-2459)
    ...
    let setupTutorialNav = function(manifestFileContent) {
        ...
        $(manifestFileContent.tutorials).each(function(i, tutorial) {
            ...
            $(document.createElement('li')).each(function() {
                ...
                $(this).attr('id', 'tut-' + shortTitle.replace(/[\(\)]+?/g, ''));
    ...
    let selectTutorial = function(manifestFileContent, position=0) {
        $('#tut-' + getParam(queryParam).toString().replace(/[\(\)]+?/g, '')).addClass('selected'); //add class selected to the tutorial that is selected by using the ID        
    ...
    let changeTutorial = function(shortTitle, anchor="") {
    ...
    /* Add the Need Help link in the footer (DBDOC-2459) */
    let addNeedHelpLink = function(support) {
        if (support !== undefined) {
            // the Need Help? URL is taken from the manifest file (key is support)
            let need_help = $(document.createElement('li')).append($(document.createElement('a')).attr({'href': support, 'target': '_blank'}).text('Need Help?'));
            $('.footer-links').append(need_help); 
        }
    }

    /* Enables collapse/expand feature for the steps */
    let setupContentNav = function() {
    ...
    // QA part of the code
    let performQA = function(articleElement, markdownContent) {
