...
Last updated: 9-Apr-2021 by Ashwin Agarwal
Version: 21.1.2
Feature added: DBDOC-2540 - Allow arrays in conditions
For example: <if type="freetier livelabs ... ...">Add multiple conditions like this.</if>
+ DBDOC-2484 - Update the Tutorial Creator tool
The tool shows the latest template now.
*/
...
let main = function() {
        ...
        $.when(
            ...
            $.getJSON(manifestFileName, function(manifestFile) {
                ...
                for (let short_name in manifestFile.include) {
                    ...
                    if (include_fname.indexOf("http") == -1) { // if the link used is relative
    ...
    let highlightCodeBlock = function(articleElement) {        
        $(articleElement).find('pre code').each(function(_, block) {
    ...
    let addNeedHelpLink = function(help, wtitle) {
        ...
        if (help !== undefined) {
            ...
            let need_help = $(document.createElement('a')).attr(
                {'href': 'mailto:' + help + '?subject=' + subject, 'title': help_text, 'id': 'need_help', 'tabindex': '0'}).text('?');
    ...
    let singlesource = function(markdownContent, type) {
        ...
        do {
            ...
            let all_types = matches[1].split(' '), matchFound = false;            
